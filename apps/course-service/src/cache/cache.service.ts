...
export class CacheService implements OnModuleInit {
  ...
  onModuleInit() {
    // this.initRedis(); // Temporarily disabled for stability
    console.log('[Cache] Running in In-Memory mode (Redis bypassed).');
  }
  ...
  private initRedis() {
    ...
    try {
      ...
      console.warn(
        '[Redis] Cache initialization failed, using In-Memory mode only.',
      );
    }
  }
  ...
  async getOrSet<T>(
    ...
    if (this.isRedisReady && this.redisClient) {
      this.redisClient
        .set(key, JSON.stringify(data), 'PX', ttlMs)
        .catch((err) => {
