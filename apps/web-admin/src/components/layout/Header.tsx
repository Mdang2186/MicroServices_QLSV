...
import { getRoleBasePath, normalizeRole } from "@/lib/roles";
...
export default function Header({
    ...
    if (!propUserName) {
      try {
        ...
        if (userCookie) {
          ...
          const isScientificDegree = ["ThS", "TS", "PGS", "GS"].some((d) =>
            parsed.degree?.includes(d),
          );
          const displayName = isScientificDegree
            ? `${parsed.degree}. ${name}`
            : name;
  ...
  const changePasswordPath =
    roleBasePath === "/login" ? "/login" : `${roleBasePath}/change-password`;
  const profilePath =
    userRole === "LECTURER"
      ? "/lecturer/profile"
      : roleBasePath === "/login"
    ...
    <header className="h-16 bg-white/90 backdrop-blur-xl border-b border-slate-100 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-30 shadow-sm shadow-slate-200/20 transition-all duration-300">
      ...
      <div className="flex items-center gap-3 lg:gap-6">
        ...
        <div className="hidden md:flex items-center relative group">
          <Search
            size={16}
            className="absolute left-3.5 text-slate-400 group-focus-within:text-uneti-blue transition-colors"
          />
          <input
            type="text"
          ...
          <div className="absolute right-3 hidden group-focus-within:flex items-center gap-1">
            <span className="text-[10px] font-black text-slate-300 border border-slate-100 px-1.5 py-0.5 rounded-md">
              ESC
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 lg:gap-4">
          ...
          <div className="relative">
            ...
            >
              <div
                className={`w-9 h-9 rounded-xl bg-uneti-blue/10 flex items-center justify-center text-uneti-blue transition-all overflow-hidden relative border border-uneti-blue/5 ${dropdownOpen ? "shadow-lg shadow-uneti-blue/20" : "group-hover:shadow-lg group-hover:shadow-uneti-blue/20"}`}
              >
                {displayImage ? (
                  <Image
                    src={displayImage}
              ...
              <ChevronDown
                size={14}
                className={`text-slate-300 transition-all hidden sm:block ${dropdownOpen ? "rotate-180 text-slate-600" : "group-hover:text-slate-600"}`}
              />
            </button>
