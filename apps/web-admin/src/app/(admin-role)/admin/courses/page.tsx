...
export default function AdminCoursesPage() {
    ...
    const filteredAdminClasses = useMemo(() => {
        if (!selectedSubjectId) return adminClasses;
        const subj = subjects.find(s => s.id === selectedSubjectId);
        const majorId = subj?.majorId;
        return majorId ? adminClasses.filter(ac => ac.majorId === majorId) : adminClasses;
    }, [adminClasses, subjects, selectedSubjectId]);
    ...
    useEffect(() => {
        const fetchPreviews = async () => {
            ...
            try {
                if (form.lecturerId) {
                    const res = await fetch(API(`/courses/schedule/lecturer/${form.lecturerId}?semesterId=${selectedSemesterId}`), { headers });
    ...
    const generateCode = () => {
        const subj = subjects.find(s => s.id === selectedSubjectId);
        const count = coursesForSubject.length + 1;
        return `${subj?.code || "LHP"}_${String(count).padStart(2, "0")}`;
    };
    ...
    const toggleAdminClass = (id: string) => {
        setForm(p => ({
            ...p,
    ...
    return (
        <div className={`h-[calc(100vh-140px)] flex flex-col gap-5 text-${THEME.textMain}`}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-xl font-bold tracking-tight text-indigo-900 uppercase">Quản trị: Quản lý Học phần</h1>
                    <p className={`text-xs text-${THEME.textMuted}`}>Thiết lập danh mục lớp và thời khóa biểu hệ thống</p>
                </div>
                    ...
                    {showDetail ? (
                        <div className="flex-1 flex flex-col overflow-hidden animate-in fade-in slide-in-from-right-4 duration-300">
                            <div className={`px-6 py-4 border-b border-${THEME.bg} flex items-center justify-between gap-4`}>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        className={`px-5 py-2 bg-${THEME.primary} text-white rounded-lg text-xs font-bold hover:opacity-90 transition-all disabled:opacity-50 shadow-md shadow-indigo-100`}
                                    >
                                        {formLoading ? "Đang xử lý..." : "Lưu dữ liệu"}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                                ...
                                {activeCourse && (
                                    <div className="bg-gradient-to-br from-indigo-50/50 to-slate-50/50 border border-indigo-100/50 rounded-2xl p-5 mb-8 animate-in fade-in slide-in-from-top-4">
                                        <div className="flex flex-wrap items-center gap-8">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Mã học phần</p>
                                                    <p className="text-sm font-black text-slate-900 tracking-tight">{activeCourse.subject?.code || 'N/A'}</p>
                                                </div>
                                            </div>

                                            <div className="h-10 w-px bg-slate-200/60 hidden md:block"></div>

                                            <div>
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Tình trạng sĩ số</p>
                                                <div className="flex items-center gap-3">
                                                    <div className="w-32 h-2 bg-slate-200 rounded-full overflow-hidden">
                                                        <div 
                                                            className={`h-full transition-all duration-1000 ${((activeCourse.currentSlots || 0) / (activeCourse.maxSlots || 60)) > 0.9 ? 'bg-rose-500' : 'bg-uneti-blue'}`}
                                                            style={{ width: `${Math.min(100, ((activeCourse.currentSlots || 0) / (activeCourse.maxSlots || 60)) * 100)}%` }}
                                                        />
                                                    </div>
                                            ...
                                            <div>
                                                ...
                                                <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${activeCourse.status === 'OPEN' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                                                    {activeCourse.status === 'OPEN' ? 'Đang mở' : 'Đã khóa'}
                                                </span>
                                            </div>
                                            ...
                                            <div>
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Học kỳ áp dụng</p>
                                                <div className="flex items-center gap-2 text-indigo-700">
                                                    <Calendar size={14} />
                                                    <span className="text-[11px] font-black">{activeCourse.semester?.name || 'Học kỳ hiện tại'}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className={`p-4 bg-${THEME.bg} rounded-xl border border-${THEME.border} flex flex-wrap gap-2`}>
                                        {filteredAdminClasses.map(ac => (
                                            <button
                                                ...
                                                type="button"
                                                disabled={isLocked}
                                                onClick={() => toggleAdminClass(ac.id)}
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-3">
                                            {form.schedules.map((s, i) => (
                                                <ScheduleRow key={i} schedule={s} index={i} rooms={rooms} disabled={isLocked} onChange={updateSchedule} onRemove={removeSchedule} />
                                            ))}
                                    ...
                                    {(form.lecturerId || form.adminClassIds.length > 0) && (
                                        <div className="pt-6 border-t border-slate-100 animate-in fade-in slide-in-from-bottom-4">
                                            ...
                                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                                ...
                                                <div>
                                                    <div className="flex items-center justify-between mb-2">
                                                        <span className="text-[10px] font-bold text-slate-500 uppercase">Lịch học các lớp hành chính</span>
                                                        {previewLoading && <div className="w-3 h-3 border-2 border-slate-200 border-t-indigo-500 rounded-full animate-spin" />}
                                                    </div>
                                    ...
                                    {coursesForSubject.map(c => (
                                        <button key={c.id} onClick={() => setSelectedCourseId(c.id)} className={`p-4 bg-white border border-${THEME.border} rounded-xl text-left hover:border-${THEME.primary}/30 hover:shadow-md transition-all group`}>
                                            ...
                                            <div className="mt-3 text-[10px] text-slate-500 space-y-1">
                                                <div className="flex items-center gap-1.5"><Users size={12} className="opacity-70" /> {c.adminClasses?.map((a: any) => a.code).join(", ") || "Chưa ghép lớp"}</div>
                                                <div className="flex items-center gap-1.5"><Calendar size={12} className="opacity-70" /> {c.schedules?.length || 0} buổi/tuần</div>
                                            </div>
                                        </button>
                                    ))}
