...
import {
    X, ShieldCheck, Loader2, Lock, Trash2, Zap, Calendar, 
    Users, AlertTriangle, CheckCircle2, XCircle, ClipboardList,
    Building, ChevronRight, RefreshCw, UserCheck, ClipboardCheck,
    MapPin, Clock, AlertCircle, CheckCheck, Bell
} from "lucide-react";
...
export default function ClassSetupPanel({
    activeCourse, selectedSubjectId, isCreating, isLocked,
    rooms, lecturers, adminClasses, headers, onClose, onSaveSuccess
    ...
    const [form, setForm] = useState({
        ...
        periodsPerSession: 3, sessionsPerWeek: 1,
    });
    ...
    useEffect(() => {
        if (activeCourse) {
            ...
            setForm({ name: "", lecturerId: "", maxSlots: 60, status: "OPEN", adminClassIds: [], periodsPerSession: 3, sessionsPerWeek: 1 });
            setSessions([]); setStudents([]); setExamRooms([]);
        }
    ...
    const fetchStudents = async (courseId: string) => {
        ...
        try {
            const res = await fetch(`/api/enrollments/admin/classes/${courseId}/enrollments`, { headers });
            const data = await res.json();
    ...
    const handleAutoSchedule = async () => {
        ...
        setFormLoading(true); setConflicts([]);
        try {
            const res = await fetch(`/api/semester-plan/schedule/${activeCourse.semesterId}`, {
                method: "POST",
                headers: { "Content-Type": "application/json", ...headers },
                body: JSON.stringify({ classId: activeCourse.id, periodsPerSession: form.periodsPerSession, sessionsPerWeek: form.sessionsPerWeek })
            });
            if (res.ok) {
                fetchSessions(activeCourse.id); onSaveSuccess();
            } else {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const body = activeCourse ? form : { ...form, subjectId: selectedSubjectId };
    ...
    const handleDelete = async () => {
        ...
        } catch { } finally { setFormLoading(false); setDeleteConfirm(false); }
    };
    ...
    const syncStudentsFromAdminClass = async () => {
        ...
        setStudentLoading(true);
        try {
            const res = await fetch(`/api/courses/${activeCourse.id}/sync-enrollments`, {
                method: "POST", headers
            });
    ...
    const handlePlanExam = async () => {
        if (!activeCourse || !examDate) { alert("Chọn ngày thi trước."); return; }
        setExamPlanning(true);
        try {
            // Fetch eligible students
            const gradeRes = await fetch(`/api/grades/class/${activeCourse.id}`, { headers });
            ...
            eligible.forEach(g => { initAtt[g.studentId] = !g.isAbsentFromExam; });
            ...
            for (const room of suitableRooms) {
                ...
                const batch = eligible.slice(studentIdx, studentIdx + Math.min(perRoom, room.capacity || perRoom));
            ...
            if (studentIdx < eligible.length && assigned.length > 0) {
                assigned[assigned.length - 1].students.push(...eligible.slice(studentIdx));
            }
            setExamRooms(assigned);
        } catch { } finally { setExamPlanning(false); }
    };
    ...
    const handleSaveAttendance = async () => {
        ...
        try {
            // Build grade updates
            const updates = Object.entries(examAttendance).map(([studentId, present]) => ({
                studentId,
        ...
        } catch { } finally { setSavingAttendance(false); }
    };

    // ===== COMPUTED =====
    const filteredAdminClasses = adminClasses.filter(ac =>
        ac.code.toLowerCase().includes(adminClassSearch.toLowerCase())
    ).slice(0, 20);

    const eligibleCount = useMemo(() =>
        examRooms.reduce((a, r) => a + r.students.length, 0), [examRooms]
    );
    const absentCount = Object.values(examAttendance).filter(v => !v).length;

    // ===== STYLES =====
    const labelCls = "text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block";
    const inputCls = "w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2.5 text-[11px] font-bold outline-none focus:ring-2 focus:ring-uneti-blue/10 focus:border-uneti-blue transition-all";
        ...
        <div className="w-[680px] bg-white border-l border-slate-100 flex flex-col shadow-2xl animate-in slide-in-from-right duration-500 overflow-hidden relative z-40 shrink-0">
            ...
            <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white/95 backdrop-blur-xl z-20 shrink-0">
                <div className="flex items-center gap-4 min-w-0">
                    <button onClick={onClose} className="w-9 h-9 flex items-center justify-center bg-slate-100 hover:bg-rose-50 rounded-xl transition-all shrink-0">
                ...
                {TABS.map(tab => (
                    <button key={tab.id} onClick={() => setViewTab(tab.id)}
                        className={cn(
                            ...
                            viewTab === tab.id ? "border-uneti-blue text-uneti-blue bg-white" : "border-transparent text-slate-400 hover:text-slate-600"
                        )}>
                ...
                {viewTab === "config" && (
                    <div className="p-6 space-y-6">
                        ...
                        <div className="p-5 bg-slate-900 rounded-2xl text-white flex items-center justify-between">
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-widest">Sẵn sàng đồng bộ</p>
                                <p className="text-[9px] font-bold text-emerald-400 uppercase mt-0.5 opacity-80">Dữ liệu thời gian thực</p>
                            </div>
                            <button onClick={handleSave} disabled={formLoading || isLocked}
                                ...
                                {formLoading ? <Loader2 size={14} className="animate-spin" /> : "Lưu thay đổi"}
                            </button>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input className={inputCls} value={form.name}
                                    onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Nhập tên lớp..." />
                            </div>
                            ...
                            <div>
                                ...
                                <select className={inputCls} value={form.lecturerId}
                            ...
                            <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block">Số buổi / Tuần</label>
                                    <input type="number" className={cn(inputCls, "bg-white")} value={form.sessionsPerWeek}
                                        onChange={e => setForm(p => ({ ...p, sessionsPerWeek: parseInt(e.target.value) }))} />
                                </div>
                                <div>
                                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block">Tiết / Buổi</label>
                                    <input type="number" className={cn(inputCls, "bg-white")} value={form.periodsPerSession}
                                        onChange={e => setForm(p => ({ ...p, periodsPerSession: parseInt(e.target.value) }))} />
                                </div>
                            </div>
                            <div>
                                <label className={labelCls}>Lớp hành chính</label>
                                <input type="text" placeholder="Tìm lớp..." value={adminClassSearch}
                ...
                {viewTab === "schedule" && (
                    <div className="p-6 space-y-5">
                        <div className="flex items-center justify-between">
                            ...
                            <button onClick={handleAutoSchedule} disabled={formLoading || isLocked || !activeCourse}
                        ...
                        {conflicts.length > 0 && (
                            <div className="space-y-2">
                                {conflicts.map((c, i) => <ConflictBadge key={i} msg={c} />)}
                            </div>
                        )}
                        ...
                        {sessionLoading ? (
                            ...
                            <div className="space-y-2">
                                {sessions.map(s => (
                                    <div key={s.id} className="p-4 bg-white border border-slate-100 rounded-2xl flex items-center justify-between hover:border-uneti-blue/20 transition-all">
                                        <div className="flex items-center gap-4">
                                            <div className="flex flex-col items-center min-w-[40px]">
                                                <span className="text-xl font-black text-slate-800 leading-none">{format(new Date(s.date), "dd")}</span>
                                                <span className="text-[8px] font-black text-slate-400 uppercase">{format(new Date(s.date), "MMM", { locale: vi })}</span>
                                                <span className="text-[8px] font-bold text-slate-300">{format(new Date(s.date), "yyyy")}</span>
                                            </div>
                                            ...
                                            <div>
                                                <p className="text-[11px] font-black text-slate-800 flex items-center gap-2">
                                                    ...
                                                    {s.room?.capacity && <span className="text-[9px] font-bold text-slate-400">({s.room.capacity} chỗ)</span>}
                                                </p>
                                                <p className="text-[9px] font-bold text-slate-400 uppercase mt-0.5">
                                                    Tiết {s.startShift} – {s.endShift}
                                                    {s.type && <span className="ml-2 text-uneti-blue">{s.type}</span>}
                                                </p>
                                            </div>
                                        </div>
                                ...
                                {sessions.length === 0 && !sessionLoading && (
                                    <div className="py-16 text-center border-2 border-dashed border-slate-100 rounded-2xl">
                                        <Calendar size={32} className="text-slate-200 mx-auto mb-3" strokeWidth={1} />
                                        <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Chưa có lịch học</p>
                                        <p className="text-[9px] text-slate-300 font-bold mt-1">Bấm "Sinh lịch tự động" hoặc thêm thủ công</p>
                                    </div>
                                )}
                            </div>
                        )}
                ...
                {viewTab === "students" && (
                    <div className="p-6 space-y-5">
                        <div className="flex items-center justify-between">
                            ...
                            <button onClick={syncStudentsFromAdminClass} disabled={studentLoading || !activeCourse}
                                className="flex items-center gap-1.5 text-[10px] font-black text-uneti-blue uppercase tracking-wider hover:text-uneti-blue/80 disabled:opacity-30 transition-colors">
                        ...
                        {activeCourse?.adminClasses?.length > 0 && (
                            <div className="flex flex-wrap gap-2">
                                {activeCourse.adminClasses.map((ac: any) => (
                                    <span key={ac.id} className="text-[9px] font-black text-uneti-blue bg-blue-50 border border-blue-100 px-2 py-1 rounded-lg">
                                        {ac.code}
                                    </span>
                                ))}
                            </div>
                        ...
                        {studentLoading ? (
                            ...
                            <div className="space-y-2">
                                {students.map((e, idx) => (
                                    <div key={e.id} className="flex items-center gap-4 p-3 bg-white border border-slate-100 rounded-xl hover:bg-slate-50/50 transition-all">
                                        <span className="text-[10px] font-black text-slate-300 w-6 text-right">{idx + 1}</span>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-[11px] font-black text-slate-800 truncate">{e.student?.fullName}</p>
                                            <p className="text-[9px] font-bold text-uneti-blue">{e.student?.studentCode}</p>
                                        </div>
                                        <span className={cn(
                                            "text-[8px] font-black px-2 py-0.5 rounded-full border",
                                            e.status === "REGISTERED" ? "text-emerald-600 bg-emerald-50 border-emerald-100" : "text-slate-400 bg-slate-50 border-slate-100"
                                        )}>{e.status}</span>
                                    </div>
                                ))}
                                {students.length === 0 && (
                                    <div className="py-16 text-center border-2 border-dashed border-slate-100 rounded-2xl">
                                        <Users size={32} className="text-slate-200 mx-auto mb-3" strokeWidth={1} />
                                        <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Chưa có sinh viên</p>
                                        <p className="text-[9px] text-slate-300 font-bold mt-1">Đồng bộ từ lớp hành chính hoặc thêm thủ công</p>
                                    </div>
                                )}
                            </div>
                        )}
                ...
                {viewTab === "exam" && (
                    <div className="p-6 space-y-5">
                        ...
                        <div className="p-5 bg-rose-50/50 border border-rose-100 rounded-2xl space-y-4">
                            <p className="text-[9px] font-black text-rose-600 uppercase tracking-widest">Cấu hình Kỳ thi</p>
                            <div className="grid grid-cols-3 gap-3">
                                <div>
                                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block">Ngày Thi</label>
                                    <input type="date" value={examDate} onChange={e => setExamDate(e.target.value)}
                                        className={inputCls} />
                                </div>
                                <div>
                                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block">Ca Thi</label>
                                    <select value={examShift} onChange={e => setExamShift(e.target.value)}
                                        className={inputCls}>
                                ...
                                <div>
                                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block">Hình thức</label>
                            ...
                            <div className="text-[9px] font-bold text-rose-500 space-y-1 pt-1">
                                <p><ChevronRight size={10} className="inline" /> Tự luận / Trắc nghiệm → Phòng Lý thuyết, max 30 SV/phòng</p>
                                <p><ChevronRight size={10} className="inline" /> Thực hành → Phòng Lab, max 25 SV/phòng</p>
                                <p><ChevronRight size={10} className="inline" /> SV có TB TK &lt; 3.0 → Cấm thi (không có trong danh sách)</p>
                            </div>

                            <button onClick={handlePlanExam} disabled={examPlanning || !examDate}
                                ...
                                {examPlanning ? <Loader2 size={14} className="animate-spin" /> : <ClipboardList size={14} />}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex gap-3">
                                    <div className="flex-1 bg-white border border-slate-100 rounded-xl p-3 text-center">
                                        <p className="text-[9px] font-black text-slate-400 uppercase">Tổng SV đủ điều kiện</p>
                                        <p className="text-xl font-black text-slate-800">{eligibleCount}</p>
                                    </div>
                                    <div className="flex-1 bg-amber-50 border border-amber-100 rounded-xl p-3 text-center">
                                        <p className="text-[9px] font-black text-amber-600 uppercase">Vắng thi</p>
                                        <p className="text-xl font-black text-amber-700">{absentCount}</p>
                                    </div>
                                    <div className="flex-1 bg-rose-50 border border-rose-100 rounded-xl p-3 text-center">
                                        <p className="text-[9px] font-black text-rose-600 uppercase">Số phòng</p>
                                        <p className="text-xl font-black text-rose-700">{examRooms.length}</p>
                                    </div>
                                </div>

                                {/* Rooms */}
                                {examRooms.map((er, ri) => (
                                    <div key={ri} className="bg-white border border-slate-100 rounded-2xl overflow-hidden">
                                        <div className="px-5 py-3 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                <Building size={14} className="text-uneti-blue" />
                                                <span className="text-[11px] font-black text-slate-800 uppercase">{er.room?.name}</span>
                                                <span className="text-[9px] font-bold text-slate-400">({er.room?.type})</span>
                                            </div>
                                            <span className="text-[9px] font-black text-uneti-blue">{er.students.length} SV</span>
                                        </div>
                                        <div className="divide-y divide-slate-50">
                                            {er.students.map((g: any, si: number) => {
                                                const isPresent = examAttendance[g.studentId] !== false;
                                                return (
                                                    <div key={g.studentId} className={cn(
                                                        "flex items-center gap-4 px-5 py-2.5 hover:bg-slate-50/50 transition-all",
                                                        !isPresent && "bg-amber-50/40"
                                                    )}>
                                                        <span className="text-[10px] font-black text-slate-300 w-5 text-right">{si + 1}</span>
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-[11px] font-black text-slate-800 truncate">{g.student?.fullName}</p>
                                                            <p className="text-[9px] font-bold text-uneti-blue">{g.student?.studentCode}</p>
                                                        </div>
                                                        {/* Attendance Checkbox */}
                                                        <label className="flex items-center gap-2 cursor-pointer select-none">
                                                            ...
                                                            <div
                                                                onClick={() => setExamAttendance(prev => ({ ...prev, [g.studentId]: !prev[g.studentId] }))}
                                                                className={cn(
                                ...
                                <button onClick={handleSaveAttendance} disabled={savingAttendance}
                                    ...
                                    {savingAttendance ? <Loader2 size={14} className="animate-spin" /> : <CheckCheck size={14} />}
                                    Lưu Điểm Danh Thi
                                </button>
                                ...
                                <div className="p-4 bg-amber-50 border border-amber-100 rounded-xl text-[9px] font-bold text-amber-700 space-y-1">
                                    <p className="font-black uppercase tracking-wider">Quy trình sau điểm danh:</p>
                                    <p>• SV vắng thi: Điểm thi 1 = 0, chờ lịch thi lại</p>
                                    <p>• Thi lại vẫn không đạt (TK2 &lt; 5.0): Học lại học phần</p>
                        ...
                        {examRooms.length === 0 && (
                            <div className="py-16 text-center border-2 border-dashed border-slate-100 rounded-2xl">
                                <ClipboardList size={32} className="text-slate-200 mx-auto mb-3" strokeWidth={1} />
                                <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Chọn ngày thi và bấm Xếp Phòng</p>
                            </div>
                        )}
                    </div>
                )}
            ...
            <div className="px-6 py-3 border-t border-slate-100 bg-slate-50/40 flex items-center justify-between shrink-0">
                <span className="text-[8px] font-black text-slate-300 uppercase tracking-[0.2em] italic">UNETI EMS · Course Engine</span>
                <span className="text-[9px] font-bold text-slate-400">
                    {activeCourse ? `${activeCourse._count?.enrollments || 0}/${activeCourse.maxSlots || "—"} SV` : ""}
                </span>
            </div>
            ...
            {deleteConfirm && (
                <div className="fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white rounded-[32px] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95">
                        <h3 className="text-base font-black text-slate-900 mb-2 uppercase">Xác nhận xóa lớp?</h3>
                        <p className="text-[11px] text-slate-500 mb-6">Tất cả lịch học, đăng ký và điểm số của lớp này sẽ bị xóa vĩnh viễn khỏi hệ thống.</p>
                        <div className="flex gap-3">
                            <button onClick={handleDelete} className="flex-1 py-3 bg-rose-600 text-white rounded-2xl text-[10px] font-black uppercase hover:bg-rose-700 transition-all">Xóa vĩnh viễn</button>
                            <button onClick={() => setDeleteConfirm(false)} className="flex-1 py-3 bg-slate-100 text-slate-600 rounded-2xl text-[10px] font-black uppercase hover:bg-slate-200 transition-all">Hủy</button>
                        </div>
                    </div>
                </div>
            )}
