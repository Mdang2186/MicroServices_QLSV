...
import {
    ChevronRight, ArrowLeft, Save, Users, Search, AlertCircle, CheckCircle2,
    Calculator, Lock, Bell, Send, ShieldAlert, Info, RefreshCw, FileText
} from "lucide-react";
...
/** Tính trung bình của mảng (bỏ null) */
...
export default function StaffGradesDetailPage() {
        ...
        const fetchData = async () => {
            try {
                ...
                if (gradeRes.ok && classRes.ok) {
                    ...
                    const hasPrac = (classData?.subject?.practiceHours ?? 0) > 0;
                    ...
                    const merged: GradeRow[] = gradeData.map((g: any) => {
                        ...
                        while (coef1.length < credits) coef1.push(null);
                        while (coef2.length < credits) coef2.push(null);
    ...
    const handleSave = async () => {
        ...
        try {
            const payload = rows.map(r => ({
                ...
                subjectId: r.subject?.id ?? courseClass?.subjectId,
            ...
            setMessage({ text: "Có lỗi xảy ra khi lưu điểm", type: "error" });
        } finally {
            setSaving(false);
        }
    };
    ...
    const sendReminder = async () => {
        if (!courseClass?.lecturer?.userId) { alert("Giảng viên chưa có tài khoản."); return; }
        await fetch(`/api/auth/notifications`, {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${TOKEN}` },
            body: JSON.stringify({
                ...
                content: `Phòng Đào tạo nhắc nhở hoàn tất nhập điểm lớp ${courseClass.code}.`,
                type: "REMINDER"
            })
        });
        setMessage({ text: "Đã gửi nhắc nhở tới giảng viên", type: "success" });
        setTimeout(() => setMessage({ text: "", type: "" }), 3000);
    };
    ...
    const enteredExam1 = rows.filter(r => r.examScore1 !== null).length;
    ...
    return (
        <div className="min-h-screen space-y-5 pb-20 max-w-full">
            {/* ===== HEADER ===== */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2 px-1">
                <div className="space-y-1">
                    <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        <Link href="/staff/grades" className="hover:text-uneti-blue flex items-center gap-1">
                            <ArrowLeft size={12} /> Quản lý Điểm
                        </Link>
                        <ChevronRight size={10} />
                        <span className="text-uneti-blue px-2 py-0.5 bg-blue-50 rounded-lg">Bảng Điểm Chi Tiết</span>
                    </div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                        GV: {courseClass?.lecturer?.fullName || "Chưa xác định"} • {totalStudents} Sinh viên
                    </p>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 px-1">
                {[
                    { label: "Tổng SV", value: totalStudents, color: "text-slate-800", bg: "bg-white" },
            ...
            <div className="bg-white border border-slate-100 shadow-sm rounded-2xl overflow-hidden">
                {/* Table toolbar */}
                <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/60 gap-4 flex-wrap">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-[11px] font-black text-slate-800 uppercase tracking-widest">
                            BẢNG ĐIỂM TỔNG HỢP — {courseClass?.subject?.name} ({credits} Tín chỉ)
                        </h2>
                    </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full border-collapse text-[10px]" style={{ minWidth: "1400px" }}>
                        <thead>
                            ...
                            {/* Row 3: empty spacer row for visual polish */}
                            <tr className="bg-slate-50 border-b border-slate-200">
                                {/* tx */}
                                {Array.from({ length: 3 + credits * 2 + (hasPractice ? 2 : 0) + 5 }).map((_, i) => (
                                    <th key={`spacer-${i}`} className="py-0.5 border-r border-slate-100" />
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {filtered.map((row, idx) => {
                                ...
                                const total10 = row.totalScore10;
                                ...
                                return (
                                    <motion.tr key={row.id}
                                        ...
                                        <td className="py-2.5 px-3 border-r border-slate-50 min-w-[140px]">
                                            <p className="font-black text-slate-800 text-[11px] leading-tight">{row.student?.fullName}</p>
                                            <p className="text-uneti-blue font-bold text-[9px] mt-0.5">{row.student?.studentCode}</p>
                                        </td>
                                        {/* TC */}
                                        <td className="py-2.5 px-2 text-center font-black text-slate-500 border-r border-slate-50">
                                            {credits}
                                        </td>
                                        {/* CC */}
                                        ...
                                        <td className="py-2.5 px-2 text-center border-r border-slate-50 bg-rose-50/20">
                                            <ScoreInput value={row.isAbsentFromExam ? 0 : row.examScore1}
                                                disabled={!eligible || row.isAbsentFromExam}
                                                onChange={v => updateScore(row.id, "examScore1", v)} highlight="red" />
                                        </td>
                                        {/* Vắng thi */}
                                        <td className="py-2.5 px-2 text-center border-r border-slate-50 bg-rose-50/20">
                                            <input type="checkbox" checked={row.isAbsentFromExam}
                                                onChange={() => toggleAbsent(row.id)}
                                                className="w-4 h-4 rounded accent-rose-500 cursor-pointer" />
                                        </td>
                                        {/* Điểm thi 2 (thi lại) */}
                                        <td className="py-2.5 px-2 text-center border-r border-slate-100 bg-rose-50/20">
                                            <ScoreInput value={row.examScore2}
                                                disabled={eligible && !row.isAbsentFromExam && row.isPassed}
                                        ...
                                        <td className="py-2.5 px-2 text-center border-r border-slate-50 bg-emerald-50/20">
                                            <span className="font-black text-[12px] text-emerald-700">
                                                {row.finalScore1 !== null ? row.finalScore1.toFixed(2) : "—"}
                                            </span>
                                        </td>
                                        {/* TK 2 */}
                                        <td className="py-2.5 px-2 text-center border-r border-slate-100 bg-emerald-50/20">
                                            <span className="font-black text-[12px] text-emerald-600">
                                                {row.finalScore2 !== null ? row.finalScore2.toFixed(2) : "—"}
                                            </span>
                                        </td>
                                        {/* Hệ 4 */}
                                        <td className="py-2.5 px-2 text-center border-r border-slate-100 bg-slate-50">
                                            <span className="font-black text-[12px] text-slate-700">
                                                {row.totalScore4 !== null ? row.totalScore4?.toFixed(1) : "—"}
                                            </span>
                                        </td>
                                        {/* Điểm chữ */}
                                        <td className="py-2.5 px-2 text-center border-r border-slate-100 bg-slate-50">
                                            <span className={cn("font-black text-[13px]",
                                                ...
                                                row.letterGrade === "D" || row.letterGrade === "D+" ? "text-amber-600" :
                                                "text-rose-600")}>
                                                {row.letterGrade || "—"}
                                            </span>
                                        </td>
                                        ...
                                        <td className="py-2.5 px-2 text-center border-r border-slate-100 bg-slate-50">
                                            <span className={cn("text-[9px] font-black px-1.5 py-0.5 rounded-full border",
                                                passed ? "text-emerald-700 bg-emerald-50 border-emerald-100" : "text-rose-600 bg-rose-50 border-rose-100")}>
                ...
                {filtered.length === 0 && !loading && (
                    <div className="py-20 text-center">
                        <p className="text-[11px] font-black text-slate-300 uppercase tracking-widest">Không tìm thấy sinh viên</p>
                    </div>
                )}
            </div>

            {/* Legend */}
            <div className="flex flex-wrap gap-3 px-1 text-[9px] font-black uppercase tracking-wider">
                <span className="flex items-center gap-1.5 text-amber-600"><span className="w-3 h-3 rounded bg-amber-100 border border-amber-200" />CC = Chuyên Cần</span>
                <span className="flex items-center gap-1.5 text-sky-600"><span className="w-3 h-3 rounded bg-sky-100 border border-sky-200" />TX = Thường Kỳ</span>
                <span className="flex items-center gap-1.5 text-indigo-600"><span className="w-3 h-3 rounded bg-indigo-100 border border-indigo-200" />Hệ số 1 ({credits} cột)</span>
                <span className="flex items-center gap-1.5 text-violet-600"><span className="w-3 h-3 rounded bg-violet-100 border border-violet-200" />Hệ số 2 ({credits} cột)</span>
                {hasPractice && <span className="flex items-center gap-1.5 text-teal-600"><span className="w-3 h-3 rounded bg-teal-100 border border-teal-200" />TH = Thực Hành</span>}
                <span className="flex items-center gap-1.5 text-orange-600"><span className="w-3 h-3 rounded bg-orange-100 border border-orange-200" />TB TK ≥ 3.0 → Được dự thi</span>
                <span className="flex items-center gap-1.5 text-emerald-600"><span className="w-3 h-3 rounded bg-emerald-100 border border-emerald-200" />TK = 40% TB TK + 60% Điểm Thi</span>
            </div>
        </div>
    );
}
