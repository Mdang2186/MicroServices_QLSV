...
export class DashboardService {
  ...
  private buildCacheKey(prefix: string, payload: Record<string, unknown> = {}) {
    return `${prefix}:${JSON.stringify(payload)}`;
  }

  private sweepExpiredCache(now = Date.now()) {
  ...
  async getStats(
    ...
    return this.remember(cacheKey, keyword ? 5000 : 15000, async () => {
      ...
      const [
        ...
        // Attendance distribution optimized
        this.prisma.grade.count({ where: { ...gradeWhere, attendanceScore: { gte: 9 } } }),
        this.prisma.grade.count({
          where: { ...gradeWhere, attendanceScore: { gte: 7, lt: 9 } },
        }),
        this.prisma.grade.count({
          where: { ...gradeWhere, attendanceScore: { gte: 5, lt: 7 } },
        }),
        this.prisma.grade.count({
          where: { ...gradeWhere, attendanceScore: { lt: 5, not: null } },
        }),
        // GPA distribution optimized
        this.prisma.student.count({ where: { ...studentWhere, gpa: { gte: 3.6 } } }),
        this.prisma.student.count({
          where: { ...studentWhere, gpa: { gte: 3.2, lt: 3.6 } },
        }),
        this.prisma.student.count({
          where: { ...studentWhere, gpa: { gte: 2.5, lt: 3.2 } },
        }),
        this.prisma.student.count({
          where: { ...studentWhere, gpa: { gte: 2.0, lt: 2.5 } },
        }),
        this.prisma.student.count({
          where: { ...studentWhere, gpa: { lt: 2.0 } },
        }),
      ...
      const attendanceDistribution = [
        { name: "Xuất sắc (9-10)", value: attendanceExc, color: "#22c55e" },
        { name: "Tốt (7-8)", value: attendanceGood, color: "#3b82f6" },
        { name: "Trung bình (5-6)", value: attendanceAvg, color: "#eab308" },
        { name: "Yếu (<5)", value: attendanceWeak, color: "#ef4444" },
      ];

      const gpaDistribution = [
        { name: "Xuất sắc (3.6 - 4.0)", value: gpaExc, color: "#8b5cf6" },
        { name: "Giỏi (3.2 - 3.59)", value: gpaGood, color: "#3b82f6" },
        { name: "Khá (2.5 - 3.19)", value: gpaFair, color: "#06b6d4" },
        { name: "Trung bình (2.0 - 2.49)", value: gpaAvg, color: "#eab308" },
        { name: "Yếu (< 2.0)", value: gpaWeak, color: "#ef4444" },
      ];
      ...
      for (const row of statusDistributionRaw) {
        ...
        else if (row.academicStatus === "WARNING")
          statusDistribution[1].value = row._count.id;
      ...
      if (targetSemesterId) {
        ...
        operationalStats.settlementPercentage =
          revenueSnapshot.totalRevenue > 0
            ? Number(
                (
                  (revenueSnapshot.paidRevenue / revenueSnapshot.totalRevenue) *
                  100
                ).toFixed(1),
              )
            : 0;
  ...
  async getSemesters() {
    return this.remember("filters:semesters", 5 * 60 * 1000, () =>
  ...
  async getFaculties() {
    return this.remember("filters:faculties", 10 * 60 * 1000, () =>
  ...
  async getFilters(facultyId?: string) {
    const [faculties, majors, intakes, semesters] = await Promise.all([
      this.getFaculties(),
      this.getMajors(facultyId),
      this.getIntakes(),
      this.getSemesters(),
    ]);
  ...
  async getMajors(facultyId?: string) {
    const cacheKey = this.buildCacheKey("filters:majors", {
      facultyId: facultyId && facultyId !== "all" ? facultyId : "all",
    });

    return this.remember(cacheKey, 10 * 60 * 1000, () =>
  ...
  async getIntakes() {
    return this.remember("filters:intakes", 10 * 60 * 1000, async () => {
